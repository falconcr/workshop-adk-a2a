...
async def run_pokemon_info_test(client: A2AClient) -> None:
    """Runs a test asking for Pokemon information."""
    ...
    print("--- ⚡ Pokemon Info Test Request ---")
    # Send Message
    response: SendMessageResponse = await client.send_message(request)
    print_json_response(response, "📥 Pokemon Info Response")
    ...
    print("--- ❔ Query Pokemon Task ---")
    # query the task
    get_request = GetTaskRequest(id=str(uuid4()), params=TaskQueryParams(id=task_id))
    get_response: GetTaskResponse = await client.get_task(get_request)
    print_json_response(get_response, "📥 Query Pokemon Task Response")


async def run_pokemon_search_test(client: A2AClient) -> None:
    """Runs a test searching for Pokemon."""

    send_message_payload = create_send_message_payload(text="Show me a list of the first 10 Pokemon")
    request = SendMessageRequest(
        id=str(uuid4()), params=MessageSendParams(**send_message_payload)
    )

    print("--- 🔍 Pokemon Search Test Request ---")
    # Send Message
    response: SendMessageResponse = await client.send_message(request)
    ...
    task_id: str = response.root.result.id
    print("--- ❔ Query Pokemon Search Task ---")
    # query the task
    get_request = GetTaskRequest(id=str(uuid4()), params=TaskQueryParams(id=task_id))
    get_response: GetTaskResponse = await client.get_task(get_request)
    print_json_response(get_response, "📥 Query Pokemon Search Task Response")


async def run_pokemon_species_test(client: A2AClient) -> None:
    """Runs a test asking for Pokemon species information."""

    send_message_payload = create_send_message_payload(text="What is the description of Charizard?")
    request = SendMessageRequest(
        id=str(uuid4()), params=MessageSendParams(**send_message_payload)
    )

    print("--- 🐉 Pokemon Species Test Request ---")
    # Send Message
    response: SendMessageResponse = await client.send_message(request)
    ...
    task_id: str = response.root.result.id
    print("--- ❔ Query Pokemon Species Task ---")
    # query the task
    get_request = GetTaskRequest(id=str(uuid4()), params=TaskQueryParams(id=task_id))
    get_response: GetTaskResponse = await client.get_task(get_request)
    print_json_response(get_response, "📥 Query Pokemon Species Task Response")


async def run_multi_turn_pokemon_test(client: A2AClient) -> None:
    """Runs a multi-turn test with Pokemon."""
    print("--- 📝 Multi-Turn Pokemon Request ---")
    # --- First Turn ---

    first_turn_payload = create_send_message_payload(text="I want to learn about a fire-type Pokemon")
    ...
    if isinstance(first_turn_response.root, SendMessageSuccessResponse) and isinstance(
        ...
        if task.status.state == TaskState.input_required and context_id:
            ...
            second_turn_payload = create_send_message_payload(
                "Tell me about Arcanine", task.id, context_id
            )
...
async def main() -> None:
    """Main function to run the Pokemon tests."""
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=timeout) as httpx_client:
            ...
            # Run different Pokemon tests
            await run_pokemon_info_test(client)
            await run_pokemon_search_test(client)
            await run_pokemon_species_test(client)
            #await run_multi_turn_pokemon_test(client)
